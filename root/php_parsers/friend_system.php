...
?><?php
if (isset($_POST['type']) && isset($_POST['user'])){//type is friend or unfriend
	$user = preg_replace('#[^a-z0-9]#i', '', $_POST['user']);
	$sql = "SELECT COUNT(id) FROM users WHERE username='$user' LIMIT 1";
	$query = mysqli_query($db_conx, $sql);//to find if user actually exists
	...
	if($_POST['type'] == "friend"){
		...
		$query = mysqli_query($db_conx, $sql);//check if already friends
		...
		$query = mysqli_query($db_conx, $sql);//check if they blocked you
		...
		$query = mysqli_query($db_conx, $sql);//check if you blocked them
		...
		$query = mysqli_query($db_conx, $sql);//if you guys are already friends...
		...
		$query = mysqli_query($db_conx, $sql);//if they are friends with you
		...
		$query = mysqli_query($db_conx, $sql);//if unaccepted request exists from you
		...
		$query = mysqli_query($db_conx, $sql);//If unaccepted request exists from them to you.
		$row_count4 = mysqli_fetch_row($query);
	    if($friend_count[0] > 99){//no one can have more than 100 frinds
            mysqli_close($db_conx);
...
if (isset($_POST['action']) && isset($_POST['reqid']) && isset($_POST['user1'])){
	//action is accept or reject (for friend request). (req id) = id of friend req that has not yet been accepted
	$reqid = preg_replace('#[^0-9]#', '', $_POST['reqid']);
	$user = preg_replace('#[^a-z0-9]#i', '', $_POST['user1']);//person who is requesting friendship
	$sql = "SELECT COUNT(id) FROM users WHERE username='$user' LIMIT 1";//check if person exists
	$query = mysqli_query($db_conx, $sql);
	$exist_count = mysqli_fetch_row($query);
	if($exist_count[0] < 1){
		mysqli_close($db_conx);
	...
	if($_POST['action'] == "accept"){
		...
		$query = mysqli_query($db_conx, $sql);
		$row_count2 = mysqli_fetch_row($query);
	    if ($row_count1[0] > 0 || $row_count2[0] > 0) {//the evaluate if they are friends already or not
		    mysqli_close($db_conx);
	        echo "You are already friends with $user.";
	        exit();
	    } else {
